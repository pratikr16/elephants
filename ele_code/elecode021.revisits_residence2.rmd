...
#'get season of entry
rev.stats$date = as.character(as.Date(rev.stats$entranceTime))
...
=======
a = ele.utm %>% mutate(date = as.character(as.Date(time))) %>% select (date, season2) %>% ungroup()

rev.stats = data.table(merge(rev.stats, a, by = "date"))
>>>>>>> Stashed changes
```
