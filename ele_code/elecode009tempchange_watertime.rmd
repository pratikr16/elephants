...
etf5 = etf3 %>% map(function(x) x = x %>% group_by(event.inv) %>% mutate(watertime = difftime(time, time[length(time)], units = "hours"), deltadw = mindw - mindw[length(mindw)]))
...
etf.merged %>% filter(watertime >= -8) %>% 
 ...
 # geom_smooth(aes(x = watertime, y = predmean, col = season2, fill = season2), lwd = 0.5)+
  ...
  geom_pointrange(aes(x = watertime, y = deltawd.mean, ymin = deltawd.mean-deltawd.ci, ymax = deltawd.mean+deltawd.ci, col = season2, shape = season2), fill = "white", fatten = 10, stroke = 0.8, lty = 1, lwd = 0.2, position = position_dodge(width = 0.3))+
  scale_colour_manual(values = c(cola,colb))+
  scale_fill_manual(values = c(cola1,colb1))+
  scale_shape_manual(values = c(21, 24))+
  theme_bw()+g1+
  labs(list(x = "Time since arrival at water (hrs)", y = "Distance to riparian zone (km)"))+
  xlim(-8,8)+
