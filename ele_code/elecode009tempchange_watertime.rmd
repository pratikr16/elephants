...
etf.fig = etf4 %>% group_by(watertime = round_any(as.numeric(watertime), 0.5), season2) %>% summarise(deltat.mean = mean(tempchange, na.rm = T), deltat.sd = sd(tempchange, na.rm = T), deltat.n = length(tempchange)) %>% mutate(deltat.ci = qnorm(0.975)*deltat.sd/sqrt(deltat.n))
```
...
etf.fig %>% 
  ...
  geom_pointrange(aes(x = watertime, y = deltat.mean, ymin = deltat.mean-deltat.ci, ymax = deltat.mean+deltat.ci, col = season2, shape = season2), fill = "white", fatten = 7, lty = 1, lwd = 0.5, position = position_dodge(width = 0.3))+
  scale_colour_manual(values = c(4,2))+
  scale_shape_manual(values = c(21, 24))+
  theme_bw()+g1+
  labs(list(x = "Time since arrival at water (hrs)", y = "ẟ Temperature °C"))
