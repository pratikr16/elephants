...
ggplot()+
    ...
    geom_rangeframe(data = data_frame(x = c(0,23), y=c(15,35)), aes(x,y))+
  ...
  scale_y_continuous(breaks = seq(15,35,10))+
  theme_pub()+
...
fig_temp_measure_relation <- ggplot()+
 # geom_smooth(aes(x = temp.a, y = mt, group = season2, lty = season2, col = season2), method = "glm", show_guide = F, lwd = 0.5,se =F, fill = "grey90")+
  ...
  geom_rangeframe(data = data_frame(x = c(5,40), y = c(5,40)), aes(x,y))+

  scale_colour_brewer(palette = "Set1")+
  scale_shape_manual(values = c(21, 24))+
  scale_x_continuous(breaks = c(5,20,40))+
  scale_y_continuous(breaks = c(5,20,40))+
  #scale_linetype_manual(values = c(2,1))+
  theme_pub()+
  coord_cartesian(xlim=c(5,40), ylim = c(5,40))+
  #coord_cartesian(xlim = c(0,40), ylim = c(0,40))+
  labs(list(x = "Ambient temperature (°C)", y = "Thermochron temperature (°C)", title = "(b)"))
...
ele.ba.test <- ele.ba.test %>%
  mutate(mean = (temp.a), diff = mean.ele.temp - temp.a)

library(mgcv)
ele.ba.gam <- gam(diff ~ s(temp.a, k = 3), data = ele.ba.test)
...
ggplot()+
  geom_segment(aes(x = 5, xend = 40, y = 0, yend =0), lty = 2, col = 1)+
  
  geom_rect(aes(xmin = c(5), xmax = c(40), ymin = c(13.38, -2.198, 5.672), ymax = c(13.77, -1.805, 5.902)), 
            fill = brewer.pal(3,"Dark2")[c(1,1,3)], 
            alpha = 0.6)+
  
 # geom_hline(yintercept = c(13.577, -2.00, 5.787), lty = 2, col = 1, size = 0.2)+
  
  geom_point(data = ele.ba.test, aes(x = (temp.a), y = - temp.a + mean.ele.temp), alpha = 0.2, size = 0.3, col = "grey50", shape = 20)+
  
  geom_line(data = ele.ba.test, aes(x = temp.a, y = pred), col = 2, size = 0.5)+
  
  geom_line(data = ele.ba.test, aes(x = temp.a, y = pred-ci), col = 2, lty = 3, size = 0.5)+
  
  geom_line(data = ele.ba.test, aes(x = temp.a, y = pred+ci), col = 2, lty = 3, size = 0.5)+
  
  #geom_text(aes(x = 40, y = c(15, 5, -4), label = c("LoA ± 95% CI","Mean diff. ± 95% CI","LoA ± 95% CI")), hjust = "inward", fontface = "italic", size = 2)+
  
  geom_rangeframe(data = data_frame(x = c(5,40), y = c(-5,15)), aes(x,y))+
  ...
  theme_pub()+
  #theme_tufte()+
  labs(list(x = "Ambient temperature (°C)", y = "Thermochron temp. - ambient temp.", title = "(c)"))+
  xlim(5,40)+
  scale_x_continuous(breaks = seq(5,40,5))+
  coord_cartesian(xlim = c(5,40), ylim = c(-5,15))
...
pdf(file = "figs/fig02temp_measures.pdf",height = half*1.7, width = full)
#png(filename = "fig_thermochron_data.png",height = 1600, width = 3200, res = 400)

grid.arrange(fig_temp_measure_hour, fig_temp_measure_relation, fig_temp_bland_altman, nrow = 2,
            layout_matrix = matrix(c(1,NA,1,3,2,3,2,NA), ncol = 4))
