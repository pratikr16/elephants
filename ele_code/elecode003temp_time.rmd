...
```{r}
#'ele speed vs ambient temp
fig_review_speed_tempamb <- 
  ele.tow %>% filter(dist.tow <= 1e4) %>% 
  group_by(season2, temp.a = round(temp.a)) %>%
  summarise(v2 = mean(v, na.rm = T), sd.v = sd(v, na.rm = T), n.v = length(v)) %>% 
  mutate(v.ci = qnorm(0.975)*sd.v/sqrt(n.v)) %>% 
  
  ggplot()+
 # geom_smooth(aes(y = v2*2/1e3, x = temp.a, group = season2, fill = season2, col = season2), lwd = 0.5,se =F, fill = "grey90")+
  
  geom_pointrange(aes(x = temp.a, y = v2*2/1e3, ymin = (v2-v.ci)*2/1e3, ymax = (v2+v.ci)*2/1e3,shape = season2,  col = season2), position = position_dodge(width = 0.5), lty = 1, fill = "white", size = 0.3)+
  theme_tufte()+
  theme(legend.position = "top", panel.grid = element_line(size = 0.2, colour = "grey"))+
  coord_cartesian(ylim=c(0,.75))+
  labs(list(x = "Ambient temperature (°C)", y = "speed (km/hr)", col = "season", shape = "season"))

ggsave(filename = "fig_review_speed_temp_amb.png", fig_review_speed_tempamb, device = png())
```
