...
ele.tow = ele2 %>% filter(dist.tow <= 5e4)
```
...
ele.BA.data = ele.tow %>% ungroup()%>% 
  filter(dist.tow <=1e4) %>% 
  mutate(diff = temp-temp.a)

#'run mods in each hour
ele.BA.mods = ele.tow %>% ungroup()%>% 
  filter(dist.tow <=1e4) %>% 
  #mutate(temp.a = log(temp.a), temp = log(temp)) %>% 
  #group_by(hour, temp.a) %>% 
  dlply("hour") %>% 
  map(function(x){
    lmer(temp ~ temp.a + season2 + (1|id), data = x)
  })

ele.ba.summary = ele.BA.mods %>% map(VarCorr) %>% 
  map(function(x){
    data_frame(sd_id = as.data.frame(x) %>% .$sdcor %>% first())
}) %>% bind_rows() %>% mutate(hour = 0:23)

mean_diff = ele.BA.data %>% group_by(hour) %>% summarise(diff = mean(diff, na.rm = T))

#'make plots through map?
  ggplot()+
    geom_rangeframe(data = data_frame(x=c(0,40), y = c(0,40)), aes(x,y))+
  geom_point(data = ele.BA.data,aes(x = temp.a, y = temp), size = 0.1)+
  geom_abline(slope = 1, lty = 2, col = 2)+
  geom_abline(data = mean_diff, aes(intercept =diff, slope = 1), col= 4)+
  #geom_abline(data = ele.ba.summary, aes(intercept = 1.96*sd_id+mean_diff$diff, slope=1), col = 3)+
    coord_cartesian(ylim = c(0,40), xlim = c(0,40))+
  facet_wrap(~hour, nrow = 4)+
    theme_pub()+
      labs(list(x = "Ambient temperature (°C)", y = "Collar temperature (°C)"))
  
#  dlply("hour") %>% 
#  map(function(x){
#    ggplot(x)+
#      geom_point(aes(x = mean.measures, y = diff.measures))+
#      geom_abline(slope = 0)+
#      geom_hline(yintercept = c(mean(x$diff.measures),
#                                mean(x$diff.measures) + first(x$sd_id)*1.96,
#                                mean(x$diff.measures) - first(x$sd_id)*1.96))+
...
#figBA_revised = 
  ...
  geom_point(data = ele.BA.data, aes(x = temp.a, y = temp_mean_wo_na), col = "transparent", shape = 21, size = 1, fill = 1)+
    
    
 # geom_text(data = ele.BA.data, aes(x = temp.a -0.2, y = mindw_mean_wo_na+0.5, label =hour), size = 3, col = 1)+
