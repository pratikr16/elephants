...
ggplot()+
 # geom_smooth(aes(x = temp.a, y = mt, group = season2, lty = season2, col = season2), method = "glm", show_guide = F, lwd = 0.5,se =F, fill = "grey90")+
  ...
  geom_pointrange(data = fig_temp_rel_data, aes(x = temp.a, y =  temp_mean, ymin = temp_mean-temp_ci , ymax = temp_mean+temp_ci, shape = season, col = season), position = position_dodge(width = 0.5), lty = 1, fill = "white", size = 0.3)+
...
figBArevised = ele.BA.data %>% 
  ...
  ggplot()+
  geom_point(aes(x = mean.measures, y = diff.measures, col = season, shape = season), fill = "white", size = 2,stroke = 1)+
  scale_color_brewer(palette = "Set1")+
  scale_shape_manual(values = c(21,24))+
  geom_rangeframe(data = data_frame(x=c(10,40), y = c(0,11)), aes(x,y))+
  
  geom_hline(yintercept = c(0,mean(mean_diff$diff),  mean(mean_diff$diff)+mean(mean_diff$sd_id), mean(mean_diff$diff)-mean(mean_diff$sd_id)), lty = c(2,1,1,1), lwd = c(0.2, 0.4,0.4,0.4), col = c(1,colb,cola,cola))+
  
  theme_pub()+
  scale_y_continuous(breaks = c(0,seq(0,11, length.out = 12)))+
