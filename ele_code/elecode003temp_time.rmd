...
ggplot()+
  geom_pointrange(data = ele.tow02, aes(x = hour, y = meantemp, ymin = meantemp-t.ci, ymax = meantemp+t.ci, shape = season2,col = season2, fill = season2), shape = 21, position = position_dodge(width = 0.5), lty = 1)+

  geom_pointrange(data = towtemp, aes(x = hour, y = meantempa, ymin = meantempa-ta.ci, ymax = meantempa+ta.ci,shape = season2, fill = season2, col = season2), shape = 24, position = position_dodge(width = 0.5), lty = 1)+
...
fig01_thermochron_plot_b.col = 
  ...
  ggplot()+
  geom_smooth(aes(x = temp.a, y = mt, group = season2, lty = season2, col = season2, fill = season2), method = "glm", show_guide = F, lwd = 0.5,se =F)+
  
  geom_pointrange(aes(x = temp.a, y = mt, ymin = mt-mt.ci, ymax = mt+mt.ci,shape = season2,  col = season2), position = position_dodge(width = 0.5), lty = 1, fill = "white")+
...
## colour
```{r fig01_col}
cairo_pdf(filename = "figs/fig04temp_loggers.colour.pdf",height = 85/25.4, width = 180/25.4, fallback_resolution = 600)
library(gridExtra)
