...
ele.rev = ele.rev %>% filter(fpt < 10, residence < 100, angle > -200)
...
ggplot(ele.rev, aes(x=mindw/1e3, y = fpt, col = season2))+
 ...
 geom_pointrange(data = ele.rev2, aes(x = mindw/1e3, y = fptm, ymin = fptm - ci, ymax = fptm+ci, col = season2, shape = season2), fill = "white", fatten = 4, stroke = 0.6, lwd = 0.4, position = position_dodge(width = 0.25))+
   scale_colour_manual(values = c(cola, colb))+
   scale_fill_manual(values = c(cola1, colb1))+
    scale_shape_manual(values = c(21, 24))+
  coord_cartesian(xlim = c(0,5), ylim = c(1.9,3), expand = T)+g1+
