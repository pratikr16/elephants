...
tower.eles = ele.ambtemp %>% group_by(year_month = paste(year(time), ifelse(month(time)<10, paste(0, month(time), sep = ""), month(time)), sep = "_")) %>% count(id) %>%
  ggplot()+
  geom_tile(aes(x = id, y = year_month, fill = n), col = 1, lwd = 0.5)+
  theme_pub()+
  scale_fill_gradientn(colours  = brewer.pal(9, "Greys"))+
  theme(axis.text.x = element_text(angle = 20, hjust = 1), legend.position = "top")+
  labs(list(fill = "Fixes", x = "Individual", y = "Time (year_month)", title = "(b)"))+
  coord_flip()

```

```{r}
pdf(file = "data.densities.pdf", height = 6, width = 6)
