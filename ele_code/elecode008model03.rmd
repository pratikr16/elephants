...
#fig09 =
  ...
  ggplot()+
  geom_smooth(aes(x = temp, y = pred.mean*2, col = season2), fill = "grey90", lty = 1, lwd = 0.5)+
 # geom_ribbon(aes(x = temp, ymin = pred.mean-ci.pred, ymax = pred.mean+ci.pred, fill = season2), alpha = 0.5)+
 
  geom_pointrange(aes(x = temp, y = v.mean*2, ymin = (v.mean-v.ci)*2, ymax = (v.mean+v.ci)*2, col = season2, shape = season2), fill = "white", size = 0.3, lty = 1)+
