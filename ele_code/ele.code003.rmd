...
fig_thermochron_plot_a =  
  ggplot()+
    geom_pointrange(data = ele.tow02, aes(x = hour, y = meantemp, ymin = meantemp-t.ci, ymax = meantemp+t.ci, col = season2,shape = season2, fill = season2), position = position_dodge(width = 0.2), fatten = 5, stroke = 1, lty = 1)+

  geom_pointrange(data = towtemp, aes(x = hour, y = meantempa, ymin = meantempa-ta.ci, ymax = meantempa+ta.ci,shape = season2), position = position_dodge(width = 0.2), col = "grey20", fatten = 5, stroke = 1, lty = 1, fill = "white")+
...
ggplot(data = tibble(cor = c(diag(tempcor.dry), diag(tempcor.wet)), hour = rep(c(1:24),2), season = c(rep("dry",24), rep("wet",24))))+
  geom_point(aes(x = hour, y = cor, col = season))+
  geom_smooth(aes(x = hour, y = cor, col =season), se = F, method = "glm")+g1
```
...
fig_thermochron_plot_b = 
  ...
  ggplot()+
  geom_smooth(aes(x = temp.a, y = mt, col = season2, group = season2, fill = season2), method = "glm", show_guide = F, lwd = 1)+
  
  geom_pointrange(aes(x = temp.a, y = mt, ymin = mt-mt.ci, ymax = mt+mt.ci,  col = season2,shape = season2, fill = season2), position = position_dodge(width = 0.2), fatten = 5, stroke = 1, lty = 1)+
