...
ele.tow02 = ele %>% mutate(distclass = cut(distw, breaks = c(500,2500))) %>% #filter (dist.tow<=1e4) %>% group_by(hour = hour(time)) %>% 
...
#fig1a =  
  ggplot()+
    geom_pointrange(data = ele, aes(x = hour, y = meantemp, ymin = meantemp-t.ci, ymax = meantemp+t.ci), col = 1, fatten = 10, stroke = 0.7, lty = 1, lwd = 0.2)
