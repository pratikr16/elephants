...
ele2 = ele2 %>% mutate(temp = round_any(temp, 2)) %>% filter(temp %in% 14:40)
```
...
fig_thermochron_plot_a =  
  ggplot()+
    geom_pointrange(data = ele.tow02, aes(x = hour, y = meantemp, ymin = meantemp-t.ci, ymax = meantemp+t.ci, shape = season2,fill = season2), shape = 21, position = position_dodge(width = 0.5), fatten = 3, stroke = 0.4, lty = 1)+

  geom_pointrange(data = towtemp, aes(x = hour, y = meantempa, ymin = meantempa-ta.ci, ymax = meantempa+ta.ci,shape = season2, fill = season2), shape = 24, position = position_dodge(width = 0.5), col = "grey20", fatten = 3, stroke = 0.4, lty = 1)+

  scale_fill_manual(values = c("white", 1))+
...
fig_thermochron_plot_a.col =  
  ggplot()+
    geom_pointrange(data = ele.tow02, aes(x = hour, y = meantemp, ymin = meantemp-t.ci, ymax = meantemp+t.ci, shape = season2,col = season2, fill = season2), shape = 21, position = position_dodge(width = 0.5), fatten = 3, stroke = 0.4, lty = 1)+

  geom_pointrange(data = towtemp, aes(x = hour, y = meantempa, ymin = meantempa-ta.ci, ymax = meantempa+ta.ci,shape = season2, fill = season2, col = season2), shape = 24, position = position_dodge(width = 0.5), fatten = 3, stroke = 0.4, lty = 1)+

  scale_colour_manual(values = c(cola2, colb2))+
  scale_fill_manual(values = c("white", colb1))+
  scale_shape_manual(values = c(21,24))+
  theme_pub()+
  labs(list(x = "Hour of day", y = "Temperature (°C)", title = "(a)"))
...
bind_rows(corrs.dry, corrs.wet) %>%
  ggplot()+
  geom_point(aes(x = hour, y = estimate, shape = season, size = 1/p.value), fill = "white")+
...
#kable(corrs.dry[c("season","hour","estimate","p.value")])
...
#fig_thermochron_plot_b = 
  ...
  ggplot()+
  geom_smooth(aes(x = temp.a, y = mt, group = season2, lty = season2), col = 1, method = "glm", show_guide = F, lwd = 0.5)+
  
  geom_pointrange(aes(x = temp.a, y = mt, ymin = mt-mt.ci, ymax = mt+mt.ci,shape = season2,  fill = season2), position = position_dodge(width = 0.5), fatten = 3, stroke = 0.4, lty = 1)+
...
fig_thermochron_plot_b.col = 
  ...
  ggplot()+
  geom_smooth(aes(x = temp.a, y = mt, group = season2, lty = season2, col = season2, fill = season2), method = "glm", show_guide = F, lwd = 0.5)+
  
  geom_pointrange(aes(x = temp.a, y = mt, ymin = mt-mt.ci, ymax = mt+mt.ci,shape = season2,  col = season2), position = position_dodge(width = 0.5), fatten = 3, stroke = 0.4, lty = 1, fill = "white")+
...
## b/w
```{r fig1}
cairo_pdf(filename = "fig1temp_loggers.pdf",height = 3, width = 6, fallback_resolution = 600)
library(gridExtra)
#png(filename = "fig_thermochron_data.png",height = 1600, width = 3200, res = 400)

grid.arrange(fig_thermochron_plot_a, fig_thermochron_plot_b, nrow = 1)
...
## colour
```{r fig1}
cairo_pdf(filename = "fig1temp_loggers.colour.pdf",height = 3, width = 6, fallback_resolution = 600)
library(gridExtra)
#png(filename = "fig_thermochron_data.png",height = 1600, width = 3200, res = 400)

grid.arrange(fig_thermochron_plot_a.col, fig_thermochron_plot_b.col, nrow = 1)
