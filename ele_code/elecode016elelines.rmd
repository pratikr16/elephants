...
ele.lines = ele.data %>% filter(loopdur <= 48) %>% arrange(id, time) %>%  dlply(c("id","loop")) %>%
...
#'export
library(rgdal)
writeOGR(ele.lines, dsn = "spatial/long_loops", layer = "ele.segments.colcode", driver = "ESRI Shapefile", overwrite_layer = T)
```