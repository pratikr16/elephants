...
ele.lines = ele.data %>% filter(loopdur > 60) %>% arrange(id, time) %>%  dlply(c("id","loop")) %>%
...
#'export
library(rgdal)
writeOGR(ele.lines, dsn = "spatial/long_loops", layer = "ele.long_loops.colcode", driver = "ESRI Shapefile", overwrite_layer = T)
```