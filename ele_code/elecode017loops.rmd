...
ele.data %>%
  ggplot()+
  stat_density(aes(x = loopdur, y =..count../sum(..count..), col = season2),geom = "line", position = "identity")+
...
fig_segment_distr <-
 ...
 scale_y_continuous(breaks = c(0, 2e3, 4e3))+
  ...
  geom_rangeframe(data = data_frame(x = c(0,50), y=c(0,4e3)), aes(x,y))+

  labs(list(x = "Segment duration (hours)", y= "Positions", title = "(a)"))+
  scale_colour_brewer(palette = "Set1")+
...
ggplot()+
  ...
  geom_rangeframe(data = data_frame(x=c(0,25),y=c(0,200)), aes(x,y))+
...
ggplot(ele.day.segments %>% filter(displace/1e3 <=10))+
  stat_density(aes(x = displace/1e3, y = ..count.., lty = season, col = season), position = "identity", geom = "line")+
  geom_rangeframe(data = data_frame(x=c(0,10),y=c(0,300)), aes(x,y))+
  scale_color_brewer(palette = "Set1")+
  scale_linetype_manual(values = c(2,1))+
  xlim(0,10)+
  
  scale_x_continuous(breaks=seq(0,10,5))+
  scale_y_continuous(breaks=seq(0,300,300))+
...
grid.arrange(fig_segment_distr, fig_segment_dist_distr, fig_segment_disp_distr, fig_segment_mid_distr, ncol = 2)
...
ele.day.segments %>%
  ...
  geom_segment(aes(x = 0,xend = 10,y=0,yend=10), col = "grey")+
  ...
  geom_pointrange(aes(x = mdist/1e3, y = mdisp/1e3, group = season, ymin = (mdisp-ci)/1e3,ymax = (mdisp+ci)/1e3, shape = season, col = season), fill = "white", position = position_dodge(width = 0.8), size = 0.3)+
...
dayloopdata <- dayloopdata %>%
  select(loopprop, mindw, waterdiff, temp, loop, season2, v, hour)
```
...
fig8list = list()
for (i in 1:3){
  b <- a %>% filter(var2 == levels(var2)[i])
  breaks <- c(first(pretty(c(min(b$mean), max(b$mean)))), last(pretty(c(min(b$mean), max(b$mean)))))
fig8list[[i]] <- ggplot(b)+

  geom_point(aes(x = loopprop, y = mean, col =season2, group = interaction(variable, season2), shape = season2))+
  geom_ribbon(aes(x = loopprop, y = mean, ymin = mean-ci, ymax = mean+ci,shape = season2, fill = season2), size = 0.3, alpha = 0.2)+
 ...
 # scale_y_continuous()
  ...
  labs(list(x = NULL, y = levels(a$var2)[i], title = paste("(",letters[i],")", sep = "")))
...
```{r}
pdf(file = "figs/fig_trackvars.pdf", height = half, width= full)

library(gridExtra)

grid.arrange(fig8list[[1]],fig8list[[2]],fig8list[[3]],ncol = 3, bottom = "Segment stage", left = "Variable")
