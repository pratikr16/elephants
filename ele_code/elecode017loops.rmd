...
ele_p50_dists %>% 
  ggplot()+
  geom_histogram(aes(x = p50dist/1e3, y = ..count../sum(..count..), fill = season), position = "dodge", lwd = 0.0)+
  scale_fill_manual(values = c(cola3, colb3))+
 scale_colour_manual(values = c(1,"transparent"))+
  theme_pub()+
  xlim(0,10)+
  labs(list(x = "Distance b/w successive segment midpoints (km)", y = "Proportion of segments"))
...
ggplot()+
  geom_histogram(data = ele7, aes(x = looptime, y = ..count../sum(..count..), fill = season), position = "dodge",lwd = 0.0)+
  geom_rect(aes(xmin = 12, xmax = 24, ymin = -0.001, ymax = 0.06), col = "grey20", lwd = 0.2, fill = "transparent")+
 scale_fill_manual(values = c(cola3, colb3))+
 scale_colour_manual(values = c(1,"transparent"))+
  #facet_wrap(~ifelse(season2 == "dry", "Cool dry", "Hot wet"))+
  theme_pub()+
  labs(list(x = "Segment duration (hours)", y = "Proportion of segments"))
...
ggplot()+
  geom_histogram(data = ele7 %>% filter(looptime >12, looptime < 24), aes(x = distance/1e3, y = ..count../sum(..count..), fill = season), position = "dodge",lwd = 0.0)+
 scale_fill_manual(values = c(cola3, colb3))+
  scale_colour_manual(values = c(1, "transparent"))+
  #facet_wrap(~ifelse(season2 == "dry", "Cool dry", "Hot wet"))+
  theme_pub()+
  labs(list(x = "Segment length (km)", y = "Proportion of segments"))
...
ele7 %>% filter(looptime> 12, looptime < 24) %>% 
  group_by(mdisp = round_any(between_point_dist, 500), season) %>% 
  summarise(mdist = mean(distance, na.rm = T), sd = sd(distance, na.rm = T), n = length(distance)) %>% 
  mutate(ci = qnorm(0.975)*sd/sqrt(n)) %>% 
  ggplot(aes(x = mdisp/1e3, y = mdist/1e3, group = season))+
  geom_pointrange(aes(ymin = (mdist-ci)/1e3,ymax = (mdist+ci)/1e3, shape = season), fill = "white", position = position_dodge(width = 0.8), stroke = 0.6)+
# facet_wrap(~dur, scales = "free")+
  theme_pub()+theme(strip.text = element_text(colour = 1))+
  scale_shape_manual(values = c(21,24))+
  labs(list(y = "Distance along segment (km)", x = "Distance b/w segment start & end points"))+
  xlim(0,10)+
...
```{r}
#'export
png(filename = "speed.looptime.png", res = 300, height = 1600, width = 1600)

## there is no relation

ele7 %>% filter(looptime < 24, looptime> 12) %>%# mutate(dur = cut(looptime, c(0,3,12,24,48,72))) %>%
  ggplot(aes(x = between_point_dist/1e3, y = v*2/1e3))+
  geom_point(size = 0.4)+
  #facet_wrap(~dur, scales = "free")+
  theme_pub()+theme(strip.text = element_text(colour = 1))+
  labs(list(x = "Duration, track b/w water points (hrs)", y = "Speed (km/hr)"))
...
#fig2a = 
ggplot(ele7 %>% filter(looptime > 12, looptime < 24))+
  geom_histogram(aes(x = between_point_dist/1e3, y = ..count../sum(..count..), fill = season), position = "dodge")+
  scale_fill_manual(values = c(cola3, colb3))+
  theme_pub()+
  labs(list(x = "Displacement b/w start & end of segment (km)", y = "Proportion of segments"))+
  xlim(0,10)
