...
ele.data$watervisits <- ifelse(ele.data$mindw <=200,# & ele.data$residence >=10 & ele.data$revisits >= 10, 
1, 0)
...
ele.waterpoints <- ele.data %>% 
  ...
  bind_rows() %>% 
  na.omit() %>% 
  mutate(class = cut(wvint, breaks = c(0,1,3,12,24,48,60,72)))

ggplot()+geom_bar(data = ele.waterpoints, aes(x = class, y = ..count../sum(..count..), fill = season2))+
  labs(list(x = "water visit interval (hrs)"))+
  scale_y_continuous(sec.axis = sec_axis(trans = ~.*nrow(ele.waterpoints), name = "positions <=500m from water"))+ylab("proportion of water points")
...
#fig2b = 
ggplot()+
  stat_density(data = ele.waterpoints %>% filter(wvint %between% c(3,75)),aes(x = wvint, col = season2), position = "identity",geom = "point", lwd = 0.7)+
...
ele.data <- ele.data %>% 
  ...
  mutate(watervisits = ifelse(mindw <=200 & wvint > 2,1,0)) %>% 
  filter(!is.na(watervisits))

#'group by ele, arrange by time, and assign loop numbers
ele.data <- ele.data %>% 
  group_by(id) %>% 
  arrange(id, time) %>% 
  mutate(loop = cumsum(watervisits)) %>% ungroup()
```
...
#'diagnostic plot
ele.data %>% 
  mutate(class = cut(loopdur, breaks = c(-Inf,1,3,12,24,48,72, Inf))) %>% 
  ggplot()+
  geom_bar(aes(x = class, y =..count../sum(..count..), fill = season2))+
...
ele.all.segments %>% 
  #filter(looptime <= 75) %>% 
  ggplot(aes(x = class, fill = season, y = ..count../sum(..count..)))+
  geom_bar(bins = 0, position = "stack")+
  scale_y_continuous(sec.axis = sec_axis(trans = ~.*nrow(ele.all.segments), name = "Number of segments"))+
  scale_x_discrete(labels = c("≤ 3","(3,12]","(12,24]","(24,48]", "> 48"))+
  labs(list(x = "Segment duration (hrs, hrs]", y= "Proportion of segments"))+
...
#'first filter nas
ele.day.segments <- ele.median.segments %>% filter(class == "(12,24]") %>% arrange(id, time50)
...
fig5b.col = 
ggplot()+
  stat_density(data = ele.day.segments, aes(x = distance/1e3, y = ..density.., lty = season, col = season), position = "identity",lwd = 0.5, geom = "line")+
  scale_colour_manual(values = c(cola, colb))+
...
ggplot(ele.day.segments)+
  stat_density(aes(x = displace/1e3, y = ..density.., lty = season, col = season), position = "identity", geom = "line")+
  scale_colour_manual(values = c(cola, colb))+
  scale_linetype_manual(values = c(2,1))+
  theme_pub()+
...
```{r}
cairo_pdf(file = "figs/fig06_segment_distributions.pdf", height = 180/25.4, width = 180/25.4)
...
ele.day.segments %>% 
  ...
  geom_smooth(data = ele.day.segments,  aes(x = distance/1e3, y = displace/1e3, lty = season, col = season, fill = season),  method = "glm", formula = y~x, family = "gaussian", lwd = 0.5)+
...
dayloopdata <- ele.day.segments %>% select(id, loop) %>% left_join(ele.data, by = c("id", "loop"))
```
...
dayloopdata <- dayloopdata %>%
  select(loopprop, mindw, waterdiff, temp, loop, season2, v, hour, ambtempdiff)
```
...
dayloopsum = dayloopdata %>% 
  ...
  select(loopprop, season2, mindw, temp, v) %>% 
...
fig8 = dayloopsum %>% 
  mutate(var2 = case_when(
    ...
    variable == "temp"~ "Ambient temperature (°C)",
    variable == "v"~"Steplength (m/hr)",
    T~"other"
  ),
...
ggplot()+
  ...
  geom_pointrange(aes(x = loopprop, y = mean, ymin = mean-ci, ymax = mean+ci,shape = season2, col = season2), position = position_dodge(0.05),fill = "white")+
  scale_shape_manual(values = c(21,24))+
  scale_colour_manual(values = c(cola, colb))+
  scale_x_continuous(breaks = c(0,0.5,1), labels = c("Start", "Midpoint", "End"))+
  facet_wrap(~var2, scales = "free", strip.position = "left")+
  theme_pub()+theme(strip.placement = "outside", strip.text = element_text(face ="plain", size = 8))+
  labs(list(x = "Segment stage", y = NULL))
```
