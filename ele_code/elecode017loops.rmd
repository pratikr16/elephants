...
ele7 = ele6 %>% group_by(id, loop) %>% 
  summarise(x1 = first(xutm), y1 = first(yutm), x2 = last(xutm), y2 = last(yutm), season = first(season2), looptime = max(looptime, na.rm = T), v = mean(v, na.rm = T), t50 = temp[first(which(round_any(loopprop, 0.05) == 0.5))], temp = mean(temp, na.rm=T), tdiff50 = tempdiff[first(which(round_any(loopprop, 0.05) == 0.5))])
...
ele7 %>% filter(looptime < 240) %>% mutate(dur = cut(looptime, c(0,3,12,24,48,72))) %>%
  ggplot(aes(x = looptime, y = between_point_dist/1e3))+
  geom_point(size = 0.4)+
  facet_wrap(~dur, scales = "free")+theme_pub()+theme(strip.text = element_text(colour = 1))+
  labs(list(x = "Duration, track b/w water points", y = "Displacement b/w water points"))
...
ele7 %>% 
  ...
  ggplot()+
  geom_pointrange(aes(x = tround, y = mean, ymin = mean-sd, ymax=mean+sd, shape = season), col = 1, lwd = 0.5, position = position_dodge(width = 0.2), fill = "white")+
...
ele7 %>% filter(looptime < 240, looptime> 3) %>%# mutate(dur = cut(looptime, c(0,3,12,24,48,72))) %>%
