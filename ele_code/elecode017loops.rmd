...
ele.data <- ele.data %>% 
  ...
  mutate(watervisits = ifelse(mindw <=500 & wvint > 2,1,0)) %>% 
...
ele.data %>% 
  ungroup() %>% 
  na.omit(loopdur) %>% 
  mutate(class = cut(loopdur, breaks = c(0,1,3,12,24,48,60,75, max(loopdur)))) %>% 
  ggplot()+
...
ele.all.segments <- ele.segment.summary %>% 
  filter(loop > 0) %>% 
  ungroup() %>% 
  mutate(class = cut(looptime, breaks = c(-Inf,1,3,12,24,48,60,72,Inf))) %>% 
  filter(!is.na(class))
...
ele.median.segments <- ele.segment.summary %>% 
  filter(looptime %between% c(3,75))
...
ele.all.segments %>% 
  ...
  geom_bar()+
  scale_y_continuous(sec.axis = sec_axis(trans = ~.*nrow(ele.all.segments), name = "Number of segments"))+
  scale_x_discrete(labels = c("≤ 1","(1,3]","(3,12]","(12,24]","(24,48]","(48,60]","(60,72","> 72"))+
...
#fig5b.col = 
ggplot()+
  stat_density(data = ele.day.segments %>% filter(looptime >12, looptime < 24), aes(x = distance/1e3, y = ..density.., lty = season, col = season), position = "identity",lwd = 0.5, geom = "line")+
  scale_colour_manual(values = c(cola, colb))+
...
ele.day.segments %>% 
  ...
  geom_smooth(data = ele.day.segments,  aes(x = distance/1e3, y = between_point_dist/1e3, lty = season, col = season, fill = season),  method = "glm", formula = y~x, family = "gaussian", lwd = 0.5)+
...
#   scale_fill_manual(values = c(cola2, colb2))+
  scale_shape_manual(values = c(21,24))+
    labs(list(x = "Segment stage", y = "Temperature (°C)"))+
  scale_x_continuous(breaks = seq(0,1,0.5), labels = c("Start", paste(50,"%", sep=""), "End"))+
  #facet_wrap(~variable, scales = "free")+
  theme_pub()
