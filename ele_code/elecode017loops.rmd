...
ggplot(ele3 %>% filter(wvint > 3, wvint < 72, residence > 5))+
 ...
 scale_colour_manual(values = c(cola, colb))+
  facet_wrap(~ifelse(season2 == "dry", "Cool dry", "Hot wet"))+g2+
  labs(list(x = "Water visit interval (hrs)"))

#'remove points with low residence, and less than 3 hours apart
ele4 = ele3 %>% filter(wvint > 1, wvint < 72)

dev.copy(cairo_pdf, "fig0xrevisit_dist.pdf", height = 4, width = 4)
dev.off()
```
...
ggplot(loopdata)+
  #geom_pointrange(aes(x = loopprop, ymin = m-ci, ymax = m+ci, y = m))+
  geom_smooth(data = ele6 %>% bind_rows,aes(x = loopprop, y = v*2, col = season2, fill = season2))+
 # geom_smooth(aes(x = loopprop, y = v, col =season2, fill = season2))+
  
  scale_color_manual(values = c(cola,colb))+
  scale_fill_manual(values = c(cola1,colb1))+g2
```

# model: fpt ~ mindw

```{r}
#'load libs and run model
library(mgcv)
ele.rev$id = as.factor(ele.rev$id)

mod.fpt.mindw = bam(fpt ~ s(mindw, bs = "cr")+ season2+ s(hour, bs = "cc") + woody.density+ s(id, bs = "re"), data = ele.rev, na.action = na.omit)

summary(mod.fpt.mindw)
```
