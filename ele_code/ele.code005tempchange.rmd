...
ele02 %>% filter(abs(waterdiff) < 1000) %>%
  ...
  geom_hline(aes(yintercept = 0), col = "grey20", lty = 2, lwd = 0.3)+
    ...
    geom_pointrange(aes(x = temp, y = mean.waterdiff, ymin = mean.waterdiff-wd.ci, ymax = mean.waterdiff+wd.ci, col = season2, shape = season2), position = position_dodge(width = 0.5), size = 3)+
