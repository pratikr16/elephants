...
```{r prep_data}
ele.temp.mindw = ele2 %>% filter(!id %in% problem_eles, temp %in% 14:40, moving == T, v > 0, meanspeed > 0) %>%
  #mutate(pred = predict(mod.temp.mindw, newdata = ., scale = "response")) %>%
  group_by(season2, mindw_round = round_any(mindw/1e3, 0.25)) %>%
...
cairo_pdf(filename = "fig2.pdf", height = 4, width = 4)
fig2a =
ele.temp.mindw %>%
  ggplot()+
...
#  geom_line(aes(x = mindw_round, y = pred.mean, col = season2), lty = 1, lwd = 0.4)+
  geom_pointrange(aes(x = mindw_round, y = t.mean, ymin = t.mean-t.ci, ymax = t.mean+t.ci, col = season2, shape = season2), fatten = 7, stroke = 0.7, fill = "white", position = position_dodge(width = 0.2),lwd = 0.2, lty = 1)+
...
grid.arrange(fig2a, figxa, nrow = 1)
dev.off()
```
...
ele.speed.mindw = ele2 %>% filter(is.na(waterdiff)==F, moving == T, !id %in% problem_eles) %>% mutate(#v.pred = predict(mod.speed, newdata = ., scale = "response", allow.new.levels = T), 
