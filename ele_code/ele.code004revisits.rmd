...
ele.utm = ele %>% dlply("id") %>%
  map(function(x) x %>% dplyr::select(xutm, yutm, time, id) %>% arrange(time) %>% sample_frac(0.4)) %>% bind_rows()
...
ele.recurse = lapply(ele.utm, function(x) getRecursions(x, radius = 1000, timeunits = "hours"))
```
