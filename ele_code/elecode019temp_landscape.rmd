...
ele = as.data.frame(ele.sf) %>% filter(temp %in% c(14:40))
...
data3 = ele %>% 
  mutate(tempred = predict(mod.temp.measures, newdata = ele, allow.new.levels = T, type = "response"),
         ...
         round_resid = round_any(residual, 0.25)) %>%
  group_by(season2, round_resid) %>% 
  summarise(mean.wd = mean(waterdiff, na.rm = T),
            sd.wd = sd(waterdiff, na.rm = T),
            n.wd = length(waterdiff)) %>% 
  mutate(ci = qnorm(0.975)*sd.wd/sqrt(n.wd))
```

```{r}
ggplot(data3)+
  geom_vline(xintercept = 0)+geom_hline(yintercept = 0)+
  geom_pointrange(aes(x = round_resid, y = mean.wd, ymin = mean.wd -ci, ymax = mean.wd+ci, col = season2), position = position_dodge(width = 0.3))+ylim(-100,50)+
  labs(list(x = "model residuals", y = "change in distance to water (m)"))
```
