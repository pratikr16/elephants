...
#'get data
vegclass.vars = as.data.frame(ele.sf) %>% filter(hour %in% 6:18, temp %in% c(14:40)) %>% dplyr::select(veg, temp) %>% filter(!is.na(veg)) %>% mutate(data = "thermochron") %>% 
  bind_rows(as.data.frame(landsat_samples) %>% dplyr::select(temp = landsat_temp, veg) %>% mutate(data = "landsat"))
...
#veg1 = 
  ...
  geom_hline(data = data_fig3_bg, 
             aes(yintercept = mean),
             lty = c(2,1), lwd = 0.2)+
  geom_ribbon(data = data_fig3_bg,
              aes(x = 1:2, ymin = mean-ci, ymax = mean+ci))
  geom_boxplot(data = vegclass.vars,
               aes(x = veg, 
                   #group = c(veg, variable), 
                   y = temp, colour = data, fill = data), 
               outlier.size = 0.1, 
               notch = T, width = 0.6, 
 ...
 scale_fill_manual(values = c("grey20", "grey90"), labels = c("LANDSAT-7","Thermochron"))+
  scale_colour_manual(values = c("grey60","grey20"), guide = F)+
