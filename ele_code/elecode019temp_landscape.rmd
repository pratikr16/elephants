...
veg1 = 
  ggplot()+
  geom_segment(aes(x = seq(from = 0.5, by = 1, length.out = nrow(a)), xend = seq(from = 0.5, by = 1, length.out = nrow(a)), y = 14, yend = 16), lwd = 0.3, col = "grey")+
  geom_segment(aes(x = seq(from = 0.5, by = 1, length.out = nrow(a)), xend = seq(from = 0.5, by = 1, length.out = nrow(a)), y = 39, yend = 41), lwd = 0.3, col = "grey")+
  ...
  theme_pub()+
  coord_flip(
    ylim = c(15,40), xlim = c(0.2,18.5), expand = T
    )+
 scale_fill_manual(values = c("grey90", "white"), labels = c("thermochron (6am - 6pm)","LANDSAT-7"))+
  ...
  labs(list(x = "Landscape type (see Gertenbach 1983)", y = "Temperature (°C)",shape = "mean", fill = NULL))
```
...
#'prep data
ele = ele.sf; st_geometry(ele) = NULL; ele = ele %>% dplyr::select(temp, veg, id, hour, season2, landsat_temp, mindw, waterdiff) %>% mutate(id = as.factor(id)) %>% filter(hour %in% 6:18)
```
...
library(lme4)
#'run model
mod.temp.measures = glmer(temp ~ sqrt(landsat_temp) + log(mindw) + veg + season2 + (1|id) + hour, data = as.data.frame(ele.sf) %>% filter(hour %in% c(6:18), temp %in% c(14:40)), family = "poisson", na.action=na.exclude)
...
ele = as.data.frame(ele.sf) %>% filter(temp %in% c(14:40), hour %in% c(6:18))
...
data3 = ele %>% 
  mutate(tempred = predict(mod.temp.measures, newdata = ele, allow.new.levels = T, type = "response"),
         ...
         landsat_round = round_any(landsat_temp, 1)) %>%
  group_by(season2, landsat_round) %>% 
  summarise(mean_resid = mean(residual, na.rm = T),
            sd_resid = sd(residual, na.rm = T),
            n_resid = length(residual)) %>% 
  mutate(ci = qnorm(0.975)*sd_resid/sqrt(n_resid))
```

```{r}
library(plotly);library(htmlwidgets)

data3 = data3 %>% filter(mean.wd %between% c(-200,200))
fig = plot_ly(x=data3$landsat_round, y =data3$round_resid, z=~data3$mean.wd, color = data3$mean.wd, symbol = data3$season2, symbols = c("circle","square"), type = "scatter3d", colors = "RdYlBu",  name = "Temp") %>% layout(title = "some title",
    scene = list(
      xaxis = list(title = "landsat temp"),
      yaxis = list(title = "thermochron residual"),
      zaxis = list(title = "change in distance water")
    ), legend = list(z="some legend"))

saveWidget(fig, "fig_interactive.html")
```
