...
rand_mindw <- as.data.frame(samples) %>% 
  select(-geometry) %>% 
  mutate(distwh = distwh, distr_dry = dist_dry, distr_wet = dist_wet) %>% 
...
fig_water_distr <- 
  ggplot()+
  geom_rangeframe(data = data_frame(x=c(1e-2,10), y=c(0,.6)), aes(x,y))+
  
  stat_density(data = rand_mindw, aes(x = mindw/1e3, y = (..count../sum(..count..))*100, col = season), geom = "line", position = "identity", lty = 2, size = 0.4)+
  stat_density(data = ele, aes(x = mindw/1e3, y =(..count../sum(..count..))*100, col = season2), geom = "line", position = "identity", lty = 1, size = 0.4)+
    
  #geom_vline(xintercept = mean(data$mindw))+
  scale_color_brewer(palette = "Set1")+
  theme_pub()+
  
  scale_x_log10(breaks = c(1e-2,1e-1,0.5,1,2.5, 1e1))+
  scale_y_continuous(breaks = c(0,0.3,0.6))+
  coord_cartesian(xlim=c(1e-2,2e1))+
  labs(list(x = "Distance to water (km)",y = "Points (%)"))

ggsave(filename = "figs/fig_water_distr_log.png", fig_water_distr, device = png(),width = half, height = half);dev.off()
```
