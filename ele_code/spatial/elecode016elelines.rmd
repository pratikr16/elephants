...
ele.lines = ele6 %>% filter(looptime > 3, looptime < 72) %>% arrange(id, time) %>%  dlply(c("id","loop")) %>%
...
data = data %>% left_join(ele7, by = c("ele" = "id", "loop"))
...
#'export
library(rgdal)
writeOGR(ele.lines, dsn = ".", layer = "ele.allloops.colcode", driver = "ESRI Shapefile", overwrite_layer = T)
```