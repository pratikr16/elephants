...
m_1 <- clogit(used ~ cos(rel.angle) + temp + sqrt(water) + trees + strata(stratum), data = ele.ssf)
...
```{r time and water}
boxplot(water ~ floor(hour), data = subset(ele.ssf, used == 1), xlab = "Time of day", ylab = "Distance to water (m)")

#'some sort of gam

#'use a bam
library(mgcv)
m_time <- bam(used ~ sqrt(water) + trees + temp + water:hour, data = ele.ssf, family = binomial)
...
pred_data <- data.frame("stratum" = stratum, dist = 0,
                        trees =  0, water =  , rel.angle = 0, hour =
                          seq(0, 24, len = 101), temp = 25)
