...
```{r}
# get raster brick
env = brick(x = c(treeraster, water.raster, tempraster))

#'get layer names
names(env) = c("woods","water","temp")
```
...
round(cor(ele.ssf@data[, c("water","woods","temp")]), 2)
...
# So we pick elev, because it by itself explains the movement better has a lower AIC. clogit is the conditional logistic regression from the package survival the response is used, coded as 1, not used is 0. strata is a novel term.
```
...
m_1 <- clogit(used ~ cos(rel.angle) + temp + water + woods + strata(stratum), data = ele.ssf)
...
```{r }
am105 <- split(ele.move); am105 = am109$AM105
plot(m1_pred_raster)
...
```{r time and water}
boxplot(water ~ floor(hour), data = subset(ele.ssf, used == 1), xlab = "Time of day", ylab = "distw")

#'some sort of gam

m_time <- clogit(used ~ cos(rel.angle) + water + woods + temp +
                   water:pbs(hour, df = 5, Boundary.knots = c(0,24)) +
                   strata(stratum), data = ele.ssf)
